...
test("carries an attribution comment ahead of the mxfile, and it is valid XML", () => {
  // The credit lives in a source-only comment so it never renders into an image.
  assert.match(xml, /^<!-- Generated by architecture-diagrams[^]*-->\n<mxfile/);
  // An XML comment must not contain the "--" sequence, or parsers reject the file.
  const comment = xml.slice(xml.indexOf("<!--") + 4, xml.indexOf("-->"));
  assert.ok(!comment.includes("--"), "attribution comment must not contain a double hyphen");
});
