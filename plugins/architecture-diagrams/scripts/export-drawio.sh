...
# Attribution comment re-injected into exported .drawio/.xml. drawio-desktop
# re-serializes the model on export and drops top-level XML comments, so the
# credit added by mermaid-to-drawio.mjs is lost after a layout pass; we put it
# back here. Keep this wording in sync with CREDIT in scripts/lib/tools.mjs, and
# free of the "--" sequence (illegal inside an XML comment).
credit="Generated by architecture-diagrams · hetekce/custom-plugins · MIT"
...
# Restore the attribution comment on editable exports only (never on rasters).
# Insert it just before the first <mxfile>, and only if it is not already there,
# so re-running the export stays idempotent.
if { [ "$fmt" = "xml" ] || [ "$fmt" = "svg" ]; } && [ -f "$out" ] && ! grep -qF "$credit" "$out"; then
  case "$fmt" in
    xml) marker="<mxfile" ;;
    svg) marker="<svg" ;;
  esac
  tmp="$(mktemp)"
  awk -v credit="$credit" -v marker="$marker" '
    !done && index($0, marker) { print "<!-- " credit " -->"; done = 1 }
    { print }
  ' "$out" > "$tmp" && mv "$tmp" "$out"
fi
